...
class Transformations:
    ...
    def get_transformation(self):
        return {
            'corrosion_set': self.corrosion_transform,  # Add transformation for corrosion dataset
    ...
    def corrosion_transform(self):
        """Transformation for corrosion dataset with RGB 3D images and 2D masks."""

        train_transform = ts.Compose([
            ts.PadNumpy(size=self.scale_size),  # Pad the images to the desired scale size
            ts.ToTensor(),  # Convert to PyTorch tensor
            ts.ChannelsFirst(),  # Change channels to first dimension
            ts.RandomFlip(h=True, v=True, p=self.random_flip_prob),  # Random horizontal and vertical flip
            ts.RandomAffine(rotation_range=self.rotate_val, translation_range=self.shift_val,
                            zoom_range=self.scale_val, interp=('bilinear', 'nearest')),  # Random affine transformation
            ts.NormalizeMedicPercentile(norm_flag=(True, False)),  # Normalize based on intensity percentiles
            ts.RandomCrop(size=self.patch_size),  # Random cropping to patch size
            ts.TypeCast(['float', 'long'])  # Ensure correct data types
        ])

        valid_transform = ts.Compose([
            ts.PadNumpy(size=self.scale_size),  # Pad the validation images to the desired size
            ts.ToTensor(),  # Convert to PyTorch tensor
            ts.ChannelsFirst(),  # Change channels to first dimension
            ts.NormalizeMedicPercentile(norm_flag=(True, False)),  # Normalize based on intensity percentiles
            ts.SpecialCrop(size=self.patch_size, crop_type=0),  # Crop to the desired patch size
            ts.TypeCast(['float', 'long'])  # Ensure correct data types
        ])
